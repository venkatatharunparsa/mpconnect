...
import { insertEventRow, getLastEventForDemand, getAllEventsForDemand } from "@/server/repositories/event";
...
export async function appendEvent(input: AppendEventInput) {
  ...
  return db.transaction(async (tx) => {
    ...
    const prev = await getLastEventForDemand(tx, input.demandId!);
...
export async function verifyChain(demandId: string) {
  const rows = await getAllEventsForDemand(demandId);
...
export async function demandTimeline(demandId: string) {
  return getAllEventsForDemand(demandId);
}
