...
export function VoiceAgent() {
  ...
  useEffect(() => {
    apiFetch("/api/voice/live", { method: "OPTIONS" })
  ...
  const startCall = async () => {
    ...
    setStatus(
      liveApi
        ? "Gemini Live session startingâ€¦"
        : "Live API pending â€” push-to-talk mode. Hold the mic, describe your problem, then confirm.",
    );
  ...
  const endCall = async () => {
    ...
    setRefId(stub);
    setStatus("Intake API pending â€” demo reference generated.");
  };
  ...
  return (
    <div className="mx-auto flex max-w-md flex-col items-center py-8">
      ...
      <p className="mt-1 text-center text-sm text-slate-600">
        Toll-free experience in your browser â€” Telugu-first
      </p>
      ...
      <div className="mt-10 flex h-64 w-64 flex-col items-center justify-center rounded-full bg-gradient-to-b from-primary/10 to-primary/30 shadow-inner">
        ...
        {state === "active" && (
          <div className="text-center">
            ...
            <p className="mt-2 text-sm text-slate-600">{muted ? "Muted" : "Listeningâ€¦"}</p>
          </div>
        )}
        {state === "ended" && refId && (
