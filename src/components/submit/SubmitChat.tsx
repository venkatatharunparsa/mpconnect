...
export function SubmitChat() {
  ...
  const handleSend = useCallback(async () => {
    ...
    if (REF_PATTERN.test(trimmed)) {
      ...
      addBot(
        status
          ? `Status: ${status.status ?? "unknown"}${status.demandId ? ` · demand ${status.demandId}` : ""}`
          : `Ref ${trimmed.toUpperCase()} — lookup API pending.`,
      );
      setSending(false);
      return;
    }
  ...
  const startRecording = async () => {
    try {
      ...
      recorder.onstop = async () => {
        ...
        setMessages((m) => [
          ...m,
          { id: crypto.randomUUID(), role: "user", text: "🎤 Voice note", audioUrl: url },
        ]);
        setSending(true);
        addBot(t("thinking", locale));
        const result = await submitIntake({
  ...
  const onPhoto = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setMessages((m) => [
      ...m,
      { id: crypto.randomUUID(), role: "user", text: "📷 Photo", mediaUrl: url },
    ]);
    e.target.value = "";
  };
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-white/80">
              {t("demoIdentity", locale)} · {citizenKey.slice(0, 10)}…
            </p>
          </div>
        </div>
