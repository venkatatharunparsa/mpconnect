...
import { getStateHex, markerRadius } from "./utils";
...
function MapContent({
  ...
  wardsAvailable,
  onSelect,
}: Omit<CitizenMapProps, "loading" | "locale" | "selectedId">) {
...
export function CitizenMap({
  ...
  if (loading) {
    return (
      <div className="flex h-full items-center justify-center bg-slate-100 animate-pulse text-xs text-slate-400 font-bold uppercase tracking-wider">
        Loading public hotspots...
      </div>
    );
  }
    ...
    return (
      <div className="flex h-full flex-col items-center justify-center bg-slate-100 p-6 text-center">
        <p className="text-sm text-slate-600 font-bold">
          {locale === "te" ? "మ్యాప్ అందుబాటులో లేదు" : "Map Service Unavailable"}
        </p>
        <p className="mt-2 text-xs text-slate-400">
          {demands.length} community hotspot{demands.length !== 1 ? "s" : ""} on map
        </p>
      </div>
    );
  }
