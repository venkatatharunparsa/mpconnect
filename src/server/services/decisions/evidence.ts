...
import { narrateComparison } from "@/server/clients/gemini";
import { getDemandById, getDemandsByWardExcept, updateDemand } from "@/server/repositories/demand";
import { getDatasetRowsByWard } from "@/server/repositories/dataset";
...
export async function evidenceFor(demandId: string): Promise<EvidenceResult> {
  ...
  if (demand.evidenceNarrative) {
    const allWardData = await getDatasetRowsByWard(demand.ward ?? "");
    ...
    const competingDemands = await getDemandsByWardExcept(demand.ward ?? "", demandId);
  ...
  const updatedDemand = await getDemandById(demandId);
  if (!updatedDemand) throw new Error("Demand not found after recomputation");
  ...
  await updateDemand(demandId, { evidenceNarrative: narrative, updatedAt: new Date() });
