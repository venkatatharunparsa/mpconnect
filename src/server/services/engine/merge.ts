import { getSubmissionById, getSubmissionsByDemandId, getSubmissionsByIds, updateSubmission } from "@/server/repositories/submission";
import { getDemandById, updateDemand, createDemand } from "@/server/repositories/demand";
import { db } from "@/server/db";
import { submissions, demands } from "@/server/db/schema";
import { eq, and, ne, gte, inArray } from "drizzle-orm";
import { checkCluster } from "@/server/services/abuse/abuse-cluster";
import { CONFIG } from "@/server/core/config";
import { embedText } from "@/server/clients/gemini";
import { appendEvent } from "@/server/services/lifecycle/events";
import { checkCorroboration } from "@/server/services/decisions/corroboration";
import { recomputeRank } from "@/server/services/engine/rank";
...
export async function processSubmission(submissionId: string): Promise<void> {
  ...
  if (!submissionEmbedding) {
    ...
    await updateSubmission(sub.id, { embedding: submissionEmbedding, status: "extracted" });
  }
  ...
  for (const demand of candidateDemands) {
    ...
    const demandSubs = await getSubmissionsByDemandId(demand.id);
  ...
  if (bestDemand && bestScore >= CONFIG.merge.thetaHi) {
    // A. Auto-merge
    await updateSubmission(sub.id, { demandId: bestDemand.id, status: "merged" });
    ...
    const allSubs = await getSubmissionsByDemandId(bestDemand.id);
    ...
    await updateDemand(bestDemand.id, updateFields);
...
export async function applyMergeReviewDecision(args: {
  ...
  if (decision === "new" || !demandId) {
    ...
    await updateSubmission(sub.id, { demandId, status: "merged" });
    ...
    // Update original demand: affected count and average location coordinates
    const allSubs = await getSubmissionsByDemandId(demandId);
    const uniqueKeys = new Set(allSubs.map((s) => s.citizenKey));
    const withGeo = allSubs.filter((s) => s.lat != null && s.lng != null);
    ...
    await updateDemand(demandId, updateFields);
...
export async function split(
  ...
  const splitSubs = await getSubmissionsByIds(submissionIds);
  ...
  for (const sub of splitSubs) {
    await updateSubmission(sub.id, { demandId: newDemand.id });
  }

  // Recalculate original demand
  const originalSubs = await getSubmissionsByDemandId(demandId);
  if (originalSubs.length > 0) {
    ...
    const origin = await getDemandById(demandId);
  ...
  const newDemandSubs = await getSubmissionsByDemandId(newDemand.id);
  ...
  await updateDemand(newDemand.id, updateFieldsNew);
